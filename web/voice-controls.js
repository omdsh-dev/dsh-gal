...
(() => {
  ...
  async function speak(text,clip,force=false){
    ...
    try{
      ...
      const response=await fetch(endpoint,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({text,language:speechLang}),signal:request.signal});
