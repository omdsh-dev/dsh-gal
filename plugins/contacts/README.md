...
- insert:
    - id: dsh-contacts
      name: /path/to/dsh-gal/plugins/contacts/lib/index.js
      config:
...
The "Visible to the character" switch in the Data panel (kept in the shared dsh-gal store, `~/.dsh/gal/store.sqlite`, as document `contacts/settings`; an older `~/.dsh/contacts/settings.json` is imported once) hides the prompt section; the tools stay available for explicit lookups. Nothing is ever written to your contacts.
