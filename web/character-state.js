...
(() => {
 ...
 let emotion='neutral',busy=false,speaking=false,preview=null,reaction=null,reactionTimer=null;
 ...
 // A reaction is a beat, not a mood: it shows through the thinking loop for a
 // moment (a tool just failed) and then hands the face back.
 const state=()=>({emotion:preview||reaction||(busy?'thinking':emotion),activity:busy?'thinking':speaking?'speaking':'idle',preview:Boolean(preview)});
 ...
 window.addEventListener('gal-reaction',event=>{if(!emotions.includes(event.detail.emotion))return;reaction=event.detail.emotion;clearTimeout(reactionTimer);reactionTimer=setTimeout(()=>{reaction=null;publish();},2600);publish();});
 ...
 window.addEventListener('gal-character-changed',()=>{emotion='neutral';preview=null;reaction=null;clearTimeout(reactionTimer);test.value='auto';publish();});
