...
Pictures for the DeepSeek Harness. `image_search` finds candidates, `image_show` downloads the chosen one into the shared store (`~/.dsh/aibo/store` blobs, `images/`) and, when [Aibo](../../README.md) is loaded, presents it in the room as an image card and returns the line to embed in the reply. Remote image URLs are never pasted into replies: they break on referer checks and rot.
...
- insert:
    - id: dsh-images
      name: /path/to/aibo/plugins/images/lib/index.js
      config:
