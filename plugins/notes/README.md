...
Apple Notes for the DeepSeek Harness. A short index of your notes (counts and the five most recently edited) becomes a prompt section in any dsh session, with tools to search, read, create and append. When [Aibo](../../README.md) is loaded too, notes show up in its Data panel with folder chips, previews and a search box.
...
- insert:
    - id: dsh-notes
      name: /path/to/aibo/plugins/notes/lib/index.js
      config:
        refreshMinutes: 15    # index cache
        folders: []           # folder names to include; empty = all
        wakeApp: false        # true = a scheduled refresh may launch Notes.app
```

Reading Notes through JXA launches Notes.app, so a scheduled refresh would keep reopening it after you quit it. By default those refreshes stand down while Notes is closed and the index simply goes stale (the panel says so); the tools, the panel's Refresh button and a write still go straight to Notes. Set `wakeApp: true` to always refresh on the timer.
...
The source has a "Visible to the character" switch in the Data panel (default on). It is the only thing this plugin stores, as the `notes/settings` document in the shared aibo store (`~/.dsh/aibo/store.sqlite`); the note index is kept in memory and re-read from Notes.app. An older `~/.dsh/notes/settings.json` (or `$DSH_NOTES_DIR/settings.json`) is imported once on first load and renamed `.migrated`. Nothing is changed in Notes unless a create or append tool is called.
