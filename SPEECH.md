...
Implementation: src/speech.ts is shared by the plugin server and scripts/ui-preview.mjs. Compile TypeScript before starting the preview; the preview imports lib/speech.js. Normal build: npm run build. If the checkout's pre-existing pnpm tsc shim is broken, npx --yes --package typescript@5.9.3 tsc -p tsconfig.json also works. Start the existing backend on 4877 and run node scripts/ui-preview.mjs for the preview on 4878.
