...
iMessage and SMS for the DeepSeek Harness. The last week of conversations from Messages.app becomes a short prompt section (who is waiting for a reply, how busy the week was) plus two tools to look at threads, in any dsh session. When [dsh-gal](../../README.md) is loaded too, it shows up in its Data panel as **Messages**.
...
- insert:
    - id: dsh-messages
      name: /path/to/dsh-gal/plugins/messages/lib/index.js
      config:
...
The shared switch lives in the shared dsh-gal store (`~/.dsh/gal/store.sqlite`, document `messages/settings`). An earlier `~/.dsh/messages/settings.json` (or `DSH_MESSAGES_DIR/settings.json`) is imported once on first use and renamed `settings.json.migrated`. Nothing else is written; message data is read live from chat.db, never stored, and the Messages database is never modified.
