...
- insert:
    - id: dsh-weread
      name: /path/to/dsh-gal/plugins/weread/lib/index.js
      config:
...
The shelf, the "Visible to the character" switch and the cached highlights (up to 200 per book) live in dsh-gal's shared store (`~/.dsh/gal/store.sqlite`, documents `weread/settings`, `weread/shelf` and one `weread.highlights/<bookId>` per book); only the cookie stays in `~/.dsh/weread/cookie.json` (`DSH_WEREAD_DIR` overrides the directory) and is never shown back in the UI. An older `state.json` is imported once on first load and renamed `state.json.migrated`.
