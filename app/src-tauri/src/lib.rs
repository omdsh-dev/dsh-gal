//! dsh-gal desktop shell: a Tauri window around the dsh-gal visual-novel UI.
//!
...
//!      that runtime,
//!   3. spawns `dsh --profile web --patch <gal.patch.yml>` with the plugin
//!      mounted on 127.0.0.1:4877 — unless a dsh-gal server is already
...
const GAL_PORT: u16 = 4877;
const GAL_URL: &str = "http://127.0.0.1:4877/";
...
fn emit(app: &AppHandle, stage: &'static str, detail: impl Into<String>) {
    ...
    eprintln!("[dsh-gal] {stage}: {detail}");
    let _ = app.emit("dsh-gal://status", Status { stage, detail });
}
...
fn gal_alive() -> bool {
    ureq::get(&format!("{GAL_URL}manifest.json"))
        .config()
...
fn ensure_runtime(app: &AppHandle, runtime: &Path) -> Result<PathBuf, String> {
    ...
    if !runtime.join("package.json").exists() {
        fs::write(runtime.join("package.json"), "{\"name\":\"dsh-gal-runtime\",\"private\":true}\n").map_err(|e| e.to_string())?;
    }
...
fn write_patch(root: &Path, plugin: &Path) -> Result<PathBuf, String> {
    let patch = root.join("gal.patch.yml");
    // Computer Use is staged under plugin/plugins/computer-use and mounted next
    // to the UI plugin, so the character can operate apps from the desktop app.
    // Its switch lives in Settings; the row is only written when it was bundled.
    let computer_use = plugin.join("plugins/computer-use/lib/index.js");
    let computer_use_row = if computer_use.exists() {
        format!(
            "    - id: dsh-computer-use\n      name: {}\n",
            serde_json::to_string(&computer_use.display().to_string()).unwrap()
        )
    } else {
        String::new()
    };
    let body = format!(
        "# Generated by the dsh-gal app on every launch.\n- insert:\n    - id: dsh-gal\n      name: {}\n      config:\n        port: {}\n        character: {}\n{}",
        serde_json::to_string(&plugin.join("lib/index.js").display().to_string()).unwrap(),
        GAL_PORT,
        std::env::var("DSH_GAL_CHARACTER").unwrap_or_else(|_| {
...
fn spawn_dsh(root: &Path, dsh_bin: &Path, patch: &Path) -> Result<Child, String> {
    let log = fs::OpenOptions::new()
        ...
        .map_err(|e| e.to_string())?;
    let _ = writeln!(&log, "\n==== dsh-gal launch {:?} ====", std::time::SystemTime::now());
...
fn boot(app: AppHandle) {
    ...
    let result: Result<(), String> = (|| {
        if gal_alive() {
            emit(&app, "attach", "a dsh-gal server is already running — attaching");
            return Ok(());
        }
        ...
        Err("dsh started but the dsh-gal UI never came up on port 4877 (see dsh-web.log)".into())
    })();
    match result {
        Ok(()) => {
            emit(&app, "ready", GAL_URL);
            if let Some(window) = app.get_webview_window("main") {
                let _ = window.navigate(GAL_URL.parse().unwrap());
            }
        }
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .expect("error while building dsh-gal")
        .run(|app, event| {
