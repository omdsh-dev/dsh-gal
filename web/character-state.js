...
(() => {
 ...
 window.addEventListener('gal-activity',event=>{
 ...
 window.addEventListener('gal-beat',event=>{const activity=event.detail.activity;if(!activities.includes(activity))return;beat=activity;clearTimeout(beatTimer);beatTimer=setTimeout(()=>{beat=null;publish();},BEAT_MS[activity]||2600);publish();});
 window.addEventListener('gal-preview',event=>{preview=activities.includes(event.detail.activity)?event.detail.activity:null;test.value=preview||'auto';publish();});
 window.addEventListener('gal-busy',event=>{busy=Boolean(event.detail.busy);if(busy)setDone(false);else{tool=null;waiting=false;}publish();});
 window.addEventListener('gal-speaking',event=>{speaking=Boolean(event.detail.speaking);publish();});
 window.addEventListener('gal-character-changed',()=>{tool=null;waiting=false;beat=null;clearTimeout(beatTimer);setDone(false);preview=null;test.value='auto';publish();});
 window.addEventListener('gal-language',()=>{render();publish();});
 window.galCharacter={get state(){return state();},text,activities};render();publish();
})();
