...
try{
  ...
  for(const provider of config.catalog.filter(p=>p.key)){
    ...
    if(provider.id==='fish'){assert.equal(body.reference_id,profile.voice);assert.equal(call.options.headers.model,profile.model);}
  ...
  // Streaming: the provider body is passed through rather than buffered, so
  // playback can start before the line has finished synthesizing.
  await service.save({language:'zh',profile:{provider:'fish',model:'s2.1-pro-free',voice:'test-voice'},apiKey:'test-secret'});
  const streamed=await service.synthesize({language:'zh',text:'你好',stream:true},signal);
  assert.equal(streamed.data,undefined);assert.ok(streamed.stream instanceof ReadableStream);
  assert.equal(Buffer.from(await new Response(streamed.stream).arrayBuffer()).toString(),'ID3mock');
  const buffered=await service.synthesize({language:'zh',text:'你好'},signal);
  assert.equal(buffered.stream,undefined);assert.equal(buffered.data.toString(),'ID3mock');
